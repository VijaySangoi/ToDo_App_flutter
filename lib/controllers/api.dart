...
class api {
  ...
  Future<String> postrequest(url, header, data) async {
    ...
    var response = await client.post(uri, headers: header, body: jsonEncode(data));
  ...
  Future<String> putrequest(url, header, data) async {
    var uri = Uri.parse(url);
    var response = await client.put(uri, headers: header, body: jsonEncode(data));
    if (response.statusCode == 200) {
      final body = response.body;
      return Future.value(body);
    }
  ...
  Future<String> deleterequest(url, header, data) async {
    var uri = Uri.parse(url);
    var response = await client.delete(uri, headers: header, body: jsonEncode(data));
    if (response.statusCode == 200) {
      final body = response.body;
      return Future.value(body);
    }
